...
בעמוד כל אפליקציה יש כפתור "פתח אפליקציה" שמעביר ל-`/open/<slug>` -
נתיב **ציבורי** (בלי צורך בהתחברות, בדיוק כמו אתר רגיל) שמזרים תעבורת
HTTP ישירות לקונטיינר של האפליקציה. ה-slug נבנה אוטומטית מהשם שבחרתם
(למשל `my-cool-bot`) - נקי בלי סיומת מספרים, ורק אם השם כבר תפוס
ע"י אפליקציה אחרת מתווסף מזהה (`my-cool-bot-42`). שמות אפליקציות
חייבים להיות ייחודיים גלובלית (לא רק מול האפליקציות שלכם, מול כולם),
כדי שאף אחד לא "יגנוב" כתובת של מישהו אחר. אם האפליקציה לא רצה, או שהיא רצה אבל
לא מאזינה בכלל על הפורט הזה (למשל כי היא "רק" בוט טלגרם בלי אתר),
מוצג דף מיתוג של Serves במקום שגיאה - ואם זוהה גם טוקן טלגרם, יש שם
גם קישור ישיר לבוט.

**מגבלה נוכחית**: זו הזרמת HTTP רגיל בלבד (לא WebSocket), והנתיב הוא
תת-נתיב (`/open/<slug>/...`) ולא סאב-דומיין ייעודי - מסגרות עבודה
שמניחות שהן רצות בשורש (`/`) עלולות לשבור קישורי assets יחסיים. שיפור
עתידי אפשרי: סאב-דומיין לכל אפליקציה (`<slug>.teleboss.online`), אבל
זה דורש רשומת DNS wildcard ואישור SSL wildcard בנוסף.
