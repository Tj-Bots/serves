...
## גישה ציבורית לאתר אפליקציה - Pro/Plus בלבד

כפתור "פתח אפליקציה" (ו-`/open/<slug>`, כולל סאב-דומיין אם מופעל) זמינים
רק למשתמשי Pro/Plus. משתמש בתוכנית חינמית שמריץ גם שרת אינטרנט (לא רק
בוט) - הבוט עצמו עדיין רץ כרגיל, רק שאין לו קישור ציבורי לאתר. זה נאכף
גם ב-UI (הכפתור לא מוצג) וגם בפועל בשרת (`app/routers/proxy.py`), אז זה
לא עוקף-דפדפן.
